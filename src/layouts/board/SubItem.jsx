...
function SubItem(props) {
  ...
  return (
    <div
      className={
        `grid grid-cols-[30px_${colsWidth[0]}px_${colsWidth[1]}px_${colsWidth[2]}px_${colsWidth[3]}px_${colsWidth[4]}px_1fr] items-center ml-6 border border-borderColor` +
        (activeCell.row === idx + 1.5 ? " bg-hoverRowColor" : "") +
        (idx === 0 ? " rounded-t" : "") +
        (idx === subItemsLength - 1
          ? " rounded-b border-b-1 mb-4"
          : " border-b-0")
      }
      onMouseEnter={() => handleHover(idx + 1.5, 0)}
      onMouseLeave={() => handleHover(-1, 0)}
    >
      <div
        className="h-full p-1 py-1.5 border-r border-borderColor"
        onMouseEnter={() => handleHover(idx + 1.5, 1)}
        onMouseLeave={() => handleHover(idx + 1.5, 0)}
      >
        {activeCell.row === idx + 2 && activeCell.col === 1 ? (
      ...
      <div
        ...
        onMouseEnter={() => handleHover(idx + 1.5, 2)}
        onMouseLeave={() => handleHover(idx + 1.5, 0)}
      >
        <div className="flex items-center justify-between w-full h-full p-2 pr-1 border-r border-borderColor">
          <div className="flex items-center gap-2">
            <div
              className={
                activeCell.row === idx + 1.5 && activeCell.col === 2
            ...
            {editCell.row === idx + 1.5 && editCell.col === 2 ? (
              <input
                type="text"
      ...
      <div
        ...
        onMouseEnter={() => handleHover(idx + 1.5, 3)}
        onMouseLeave={() => handleHover(idx + 1.5, 0)}
      >
        <span className="relative flex gap-4 m-auto align-center">
          <div
            className={
              activeCell.row === idx + 1.5 && activeCell.col === 3
                ? "absolute -left-8"
      ...
      <div
        ...
        onMouseEnter={() => handleHover(idx + 1.5, 4)}
      ...
      >
        ...
        {activeCell.row === idx + 1.5 && activeCell.col === 4 && (
          <>
            <div
              className="absolute top-0 p-1 -translate-x-1/2 -translate-y-1/2 bg-white rounded-full cursor-pointer left-1/2 custom-shadow hover:bg-blue"
              style={{ borderRadius: "50%" }}
